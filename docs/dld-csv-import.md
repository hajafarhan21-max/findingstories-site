...
Area mappings live in `area-aliases.json`; project mappings live in `project-aliases.json`. Only `EXACT` and governed `VERIFIED_ALIAS` mappings may contribute to area, project, or developer metrics. Each project entry must record its developer, verification date, and verification source, and its canonical project must exist in the published project registry.

Every successful import rebuilds `generated/mappings/`. The mapping report includes exact/verified/unmapped counts, the highest-volume unmapped areas and projects, and conflicts. Similar-looking names stay `UNMAPPED`; token-overlap candidates are written only to suggestion artifacts marked `REVIEW_ONLY` and `publishable: false`. Dubai-wide aggregates may use all valid rows, while `recordsForPage` excludes unverified attribution from area, project, and developer views.
