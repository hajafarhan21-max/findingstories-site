...
// Developer names and generic real-estate words identify a portfolio, not a project.
// Keep them out of project suggestions so sibling developments fail closed.
export const NON_IDENTIFYING_PROJECT_TOKENS=Object.freeze(new Set([
  'azizi','emaar','damac','sobha','binghatti','danube','nakheel','meraas','ellington','aldar',
  'developments','development','properties','property','realty','group','holding','holdings',
  'project','projects','residence','residences','tower','towers','villas','villa','apartments','apartment'
]));
const projectSpecificTokens=(value,target)=>{const ignored=new Set(NON_IDENTIFYING_PROJECT_TOKENS);for(const token of tokens(target?.developer))ignored.add(token);return new Set([...tokens(value)].filter(token=>!ignored.has(token)&&/\p{L}/u.test(token)))};
export const projectReviewSuggestions=(sources,targets)=>sources.flatMap(source=>targets.map(target=>{const left=projectSpecificTokens(source,target),right=projectSpecificTokens(target.name,target);if(!left.size||!right.size)return null;const shared=[...left].filter(token=>right.has(token)).length;if(!shared)return null;const score=shared/Math.max(left.size,right.size);return {source,candidate:target.name,candidate_slug:target.slug,review_score:Number(score.toFixed(3)),status:'REVIEW_ONLY',publishable:false}}).filter(Boolean).sort((a,b)=>b.review_score-a.review_score).slice(0,3));
