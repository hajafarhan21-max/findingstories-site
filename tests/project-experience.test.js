...
test('publication gate requires content, provenance and an approved visual or branded fallback',()=>{
  ...
  assert.equal(projectPublicationDecision({...project,visualFallback:'NONE'}).reason,'VISUAL_NOT_GOVERNED');
...
test('themes are project-specific and enrichment never fabricates commercial values',()=>{
  ...
  assert.deepEqual(project.commercial,{startingPrice:null,paymentPlan:null,booking:null,fees:null,handover:null,availability:'REQUIRES_CONFIRMATION',verifiedAt:null});
});
