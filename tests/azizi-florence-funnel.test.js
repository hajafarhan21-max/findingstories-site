...
import { MATERIALIZED_ASSETS,decodeWebpBase64 } from '../scripts/materialize-florence-assets.mjs';
...
test('audited Florence static assets populate production slots',()=>{const assets=resolveFlorenceAssets([]);for(const slot of ['hero','overview_main','overview_thumb_1','overview_thumb_2','overview_thumb_3','residence_3br_townhouse','residence_4br_townhouse','residence_4br_villa','residence_5br_villa','residence_6br_villa','amenities_background','location_map'])assert.match(assets[slot]?.path||'',/^\/assets\/azizi-florence\/.+\.webp$/);assert.equal(assets.overview_thumb_4,undefined);assert.equal(assets.payment_plan_visual,undefined);});

test('Florence base64 sources decode and materialize as genuine WebP images',async()=>{for(const asset of MATERIALIZED_ASSETS){const image=decodeWebpBase64(await readFile(asset.source,'utf8'),asset.source);assert.ok(image.length>100_000);const decoded=await sharp(image).raw().toBuffer({resolveWithObject:true});assert.ok(decoded.info.width>0);assert.ok(decoded.info.height>0);assert.equal(decoded.data.length,decoded.info.width*decoded.info.height*decoded.info.channels);await rm(asset.output,{force:true});}await execFileAsync(process.execPath,['scripts/materialize-florence-assets.mjs']);for(const asset of MATERIALIZED_ASSETS)await access(asset.output);});
