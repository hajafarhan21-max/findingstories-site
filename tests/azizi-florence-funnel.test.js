...
import { MATERIALIZED_ASSETS,decodeImageBase64 } from '../scripts/materialize-florence-assets.mjs';
...
test('unverified bedroom types and absent location claims are not promoted',()=>{const html=renderAziziFlorence({project:{...project,attributes:{}},campaign,units:[{...unit,bedrooms:'2',unit_type:'Unverified collection'}],sources:[]});assert.doesNotMatch(html,/Unverified collection/);assert.doesNotMatch(html,/\d+ minutes/i);});
...
test('funnel public assets and API endpoints are present in the production build structure',async()=>{const [template,client,style,handler,leads]=await Promise.all([readFile('api/_lib/azizi-florence.js','utf8'),readFile('public/azizi-florence.js','utf8'),readFile('public/azizi-florence.css','utf8'),readFile('api/acquisition.js','utf8'),readFile('api/leads.js','utf8')]);assert.match(template,/href="\/azizi-florence\.css"/);assert.match(template,/src="\/azizi-florence\.js"/);assert.match(style,/\.hero/);assert.match(client,/fetch\('\/api\/acquisition\?route=events'/);assert.match(client,/fetch\('\/api\/leads'/);assert.match(handler,/route==='azizi-florence'/);assert.match(handler,/route==='events'/);assert.match(leads,/export default async function handler/);});
...
test('maps, floor plans, logos, and unclassified images are excluded from photographic placements',()=>{
  ...
  assert.doesNotMatch(html,/dddddddd-dddd-4ddd-8ddd-dddddddddddd|image-mark|>AF</);
});
...
test('verified project copy supplies only supported quick USPs and unmanifested maps never render',()=>{const supplied={...project,description:'A 30 million sq.ft community with 3 & 4 bedroom townhouses and 4, 5 & 6 bedroom villas, 25% green and open spaces and direct access to E311.'};const map={id:'eeeeeeee-eeee-4eee-8eee-eeeeeeeeeeee',filename:'florence-location-map.png',media_type:'image/png'};const html=renderAziziFlorence({project:supplied,campaign,units:[unit],sources:[map]});for(const value of ['30 Million sq.ft Master Community','3, 4, 5 &amp; 6 Bed Townhouses &amp; Villas','~25% Green &amp; Open Spaces','Direct Access to E311'])assert.match(html,new RegExp(value));assert.match(html,/Verified Florence location plan/);assert.doesNotMatch(html,/eeeeeeee-eeee-4eee-8eee-eeeeeeeeeeee/);});
...
test('audited Florence static assets populate production slots',()=>{const assets=resolveFlorenceAssets([]);for(const slot of ['hero','overview_main','overview_thumb_1','overview_thumb_2','overview_thumb_3','residence_3br_townhouse','residence_4br_townhouse','residence_4br_villa','residence_5br_villa','residence_6br_villa','amenities_background'])assert.match(assets[slot]?.path||'',/^\/assets\/azizi-florence\/.+\.webp$/);assert.equal(assets.location_map?.path,'/assets/azizi-florence/location-map.png');assert.equal(assets.overview_thumb_4,undefined);assert.equal(assets.payment_plan_visual,undefined);});

test('Florence base64 sources decode and materialize as genuine full-resolution images',async()=>{for(const asset of MATERIALIZED_ASSETS){const image=decodeImageBase64(await readFile(asset.source,'utf8'),asset.source,asset.format);assert.ok(image.length>100_000);const decoded=await sharp(image).raw().toBuffer({resolveWithObject:true});assert.ok(decoded.info.width>0);assert.ok(decoded.info.height>0);assert.equal(decoded.data.length,decoded.info.width*decoded.info.height*decoded.info.channels);if(asset.format==='png'){const official=await readFile('assets-source/azizi-florence/Florence-location-map-page10-HD.png');assert.equal(createHash('sha256').update(image).digest('hex'),createHash('sha256').update(official).digest('hex'));assert.deepEqual([decoded.info.width,decoded.info.height],[3119,2126]);}await rm(asset.output,{force:true});}await execFileAsync(process.execPath,['scripts/materialize-florence-assets.mjs']);for(const asset of MATERIALIZED_ASSETS)await access(asset.output);});

test('Florence location map renders the accessible full-screen interaction controls',()=>{const html=renderAziziFlorence({project,campaign,units:[unit],sources:[]});for(const marker of ['data-map-open','data-map-viewer','data-map-zoom-in','data-map-zoom-out','data-map-fit','data-map-close'])assert.match(html,new RegExp(marker));assert.match(html,/aria-modal="true"/);assert.match(html,/location-map\.png/);});
