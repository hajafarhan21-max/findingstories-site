...
The focused event landing page is available at `/open-house`, and the authenticated event CRM at `/admin/events`. Event APIs automatically bootstrap the schema through the existing `DATABASE_URL`/`PRODUCTION_DATABASE_URL` resolver. Initialization is advisory-locked, repeatable, preserves all lead and event records, seeds the 36 half-hour Dubai-time slots only when missing, and installs a row-locking appointment function that cannot exceed each slot's configurable capacity. The SQL migration remains available for audited/manual database workflows, but is not required for deployment startup.
...
1. Open `/api/events/slots` once to trigger the safe schema bootstrap, then open `/api/health` and confirm the database check is `ok` without any secret values.
