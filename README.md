...
- **Qualification:** `api/_lib/qualify.js` calls the OpenAI Responses API from server code using strict JSON Schema. If OpenAI is unavailable, the lead is still persisted with a clearly labelled deterministic fallback for manual review.
...
1. Push branch `ai-agents-v1` to the Git remote and open the pull request; do not merge it into `main`.
2. In Vercel, connect/import the repository if it is not already connected. Framework preset can remain **Other**; leave the build/output settings at Vercel defaults so `index.html` and `api/` are served directly.
...
4. Trigger the Git-based preview by pushing the branch, or run `vercel` (not `vercel --prod`) from this branch and choose the existing project.
