...
import { publicationDecision } from '../platform/project-publishing.js';
...
export async function loadPublicProjects(root='projects'){
  ...
  for(const entry of entries.filter(item=>item.isDirectory()).sort((a,b)=>a.name.localeCompare(b.name))){
    ...
    if(manifest.status==='approved'&&manifest.seo.indexable&&publicationDecision(manifest).public)projects.push(Object.freeze({...manifest.project,...manifest.seo,...(manifest.distribution?{distribution:manifest.distribution}:{})}));
  }
  return projects;
}
