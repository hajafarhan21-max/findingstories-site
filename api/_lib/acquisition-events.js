...
const schema=z.object({event_key:z.string().uuid(),visitor_id:z.string().uuid(),event_type:z.enum(['page_view','page_visit','cta_click','enquiry_started','enquiry_submitted','brochure_request','consultation_request','repeated_visit','property_comparison','payment_plan_interest','whatsapp_click','meeting_request','site_visit_request']),page_url:z.string().trim().startsWith('/').max(1000),page_type:z.string().trim().max(80).optional().default(''),area:z.string().trim().max(150).optional().default(''),project:z.string().trim().max(200).optional().default(''),developer:z.string().trim().max(200).optional().default(''),source:z.string().trim().max(120).optional().default('organic'),referrer:z.string().trim().max(1000).optional().default(''),utm_source:z.string().trim().max(200).optional().default(''),utm_medium:z.string().trim().max(200).optional().default(''),utm_campaign:z.string().trim().max(200).optional().default('')}).strict();
