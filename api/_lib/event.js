...
export const rsvpSchema = z.object({
  ...
  event_id:z.string().uuid(), preferred_event_date:z.string().date(), preferred_slot:z.string().uuid(), additional_requirements:clean(1500),
...
export const adminEventSchema=z.discriminatedUnion('action',[
  ...
  z.object({action:z.literal('archive_test_records'),event_id:z.string().uuid()}).strict(),
  z.object({action:z.literal('save_event'),id:z.string().uuid().optional(),name:z.string().trim().min(2).max(150),venue:z.string().trim().min(2).max(200),address:clean(500),starts_on:z.string().date(),ends_on:z.string().date(),opening_time:z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/),closing_time:z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/),slot_duration_minutes:z.coerce.number().int().min(5).max(240),default_slot_capacity:z.coerce.number().int().min(1).max(1000),status:z.enum(['DRAFT','OPEN','CLOSED','COMPLETED','TEST']),active:z.boolean(),developers_projects:clean(3000),public_description:clean(5000),is_test:z.boolean()}).strict()
]);
...
export function fallbackEvent(r){
  ...
  const event=safeText(r.event_name||'Finding Stories event',150),venue=safeText(r.event_venue||'our Dubai venue',200);
  return {lead_score:score,temperature,qualification_summary:`${temperature} event prospect seeking ${r.property_type||'Dubai property'} for ${r.purpose||'consultation'}.`,next_action:'Call to verify requirements and confirm the requested appointment.',suggested_call_opener:`Hello ${name}, this is Finding Stories regarding your ${event} RSVP.`,personalised_whatsapp_invitation:`Hello ${name}, thank you for your interest in ${event} at ${venue} on ${r.preferred_event_date}. We will contact you to confirm your appointment slot.`,appointment_confirmation_message:`Hello ${name}, your ${event} appointment is confirmed. Please reply if your plans change.`,reminder_message:`Hello ${name}, a friendly reminder about your ${event} appointment at ${venue}. We look forward to welcoming you.`,no_show_follow_up_message:`Hello ${name}, we missed you at ${event}. May we arrange a private follow-up consultation?`};
}
