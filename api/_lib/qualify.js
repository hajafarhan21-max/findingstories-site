const schema = {
  ...
  properties: {
    ...
    whatsapp_follow_up_draft: { type: 'string' }, call_opener: { type: 'string' }
  }, required: ['lead_score','temperature','qualification_summary','requirement_summary','missing_information','next_action','whatsapp_follow_up_draft','call_opener']
};
...
export function fallback(lead) {
  ...
  return { lead_score: score, temperature: score >= 45 ? 'Warm' : 'Cold', qualification_summary: 'Lead captured; AI qualification is pending configuration.', requirement_summary: [lead.purpose, lead.property_type, lead.budget].filter(Boolean).join(' · ') || 'Initial enquiry', missing_information: missing, next_action: 'Review the requirement and contact the lead with consent.', whatsapp_follow_up_draft: `Hello ${lead.name}, thank you for contacting Finding Stories. May we arrange a brief call to understand your UAE property requirement?`, call_opener: `Hello ${lead.name}, this is Finding Stories following up on your property enquiry.` };
}
