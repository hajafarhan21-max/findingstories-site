...
export async function binghattiActivationStatus(sql) {
  ...
  const records = await sql`SELECT unit,status,data_quality,is_test FROM property_inventory WHERE unit IN ('BAMH-1545','BAMH-634') ORDER BY unit`;
  const verified = records.length === binghattiAmberhallInventory.length && records.every(record => record.status === 'active' && record.data_quality === 'verified' && record.is_test === false);
