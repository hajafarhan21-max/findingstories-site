...
async function list(access,id=''){
 const rows=await access.sql`SELECT c.*,p.name project_name,p.developer,p.area,p.emirate,owner.display_name owner_name,
  ...
  (SELECT COUNT(*) FROM leads l WHERE l.campaign_id=c.id AND l.is_test=FALSE AND (l.qualified_at IS NOT NULL OR l.status='qualified'))::int qualified_leads,
...
export default async function handler(req,res){
 ...
 if(req.method==='GET'){
  ...
  const [recentLeads,tasks,sourcePerformance,agentPerformance,eoiPipeline]=await Promise.all([
   ...
   access.sql`SELECT COALESCE(NULLIF(source,''),'Unknown') name,COUNT(*)::int leads,COUNT(*) FILTER(WHERE qualified_at IS NOT NULL OR status='qualified')::int qualified,COUNT(*) FILTER(WHERE status IN ('booked','converted'))::int bookings FROM leads WHERE campaign_id=${campaign.id} AND is_test=FALSE GROUP BY 1 ORDER BY leads DESC`,
   access.sql`SELECT u.id,u.display_name name,COUNT(l.id)::int leads,COUNT(l.id) FILTER(WHERE l.qualified_at IS NOT NULL OR l.status='qualified')::int qualified,COUNT(l.id) FILTER(WHERE l.status IN ('booked','converted'))::int bookings,COALESCE(SUM(l.attributed_revenue) FILTER(WHERE l.status IN ('booked','converted')),0) revenue FROM crm_campaign_assignments a JOIN crm_users u ON u.id=a.user_id LEFT JOIN leads l ON l.owner_id=u.id AND l.campaign_id=a.campaign_id AND l.is_test=FALSE WHERE a.campaign_id=${campaign.id} GROUP BY u.id,u.display_name ORDER BY leads DESC`,
