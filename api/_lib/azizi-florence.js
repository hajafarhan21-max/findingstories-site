...
export const AZIZI_FLORENCE_ASSET_MANIFEST=Object.freeze({
  ...
  location_map:'/assets/azizi-florence/location-map.png'
});
...
export function resolveFlorenceAssets(sources=[],manifest=AZIZI_FLORENCE_ASSET_MANIFEST){
  const byExactReference=reference=>{
    ...
    if(typeof reference==='string')return {path:reference,filename:reference.split('/').at(-1),media_type:reference.endsWith('.png')?'image/png':'image/webp'};
...
export function aziziStructuredData(project,units,origin,seo={}){
  ...
  const listing={'@type':'RealEstateListing','@id':`${url}#listing`,name:project.name,url,description:project.description||seo.description||undefined,address:project.area||project.emirate?{'@type':'PostalAddress',addressLocality:project.area||undefined,addressRegion:project.emirate||undefined,addressCountry:'AE'}:undefined,offers:offers.length?offers:undefined,brand:project.developer?{'@type':'Organization',name:project.developer}:undefined};
  const faq=seo.faq?.length?{'@type':'FAQPage','@id':`${url}#faq`,url,mainEntity:seo.faq.map(item=>({'@type':'Question',name:item.question,acceptedAnswer:{'@type':'Answer',text:item.answer}}))}:null;
  const breadcrumb={'@type':'BreadcrumbList','@id':`${url}#breadcrumb`,itemListElement:[{'@type':'ListItem',position:1,name:'Finding Stories',item:`${origin.replace(/\/$/,'')}/`},{'@type':'ListItem',position:2,name:'Azizi Florence',item:url}]};
  const webPage={'@type':'WebPage','@id':`${url}#webpage`,url,name:seo.title||project.name,description:seo.description||project.description||undefined,breadcrumb:{'@id':`${url}#breadcrumb`},about:{'@id':`${url}#listing`},isPartOf:{'@id':`${origin.replace(/\/$/,'')}/#website`}};
  return {'@context':'https://schema.org','@graph':[webPage,breadcrumb,listing,...(faq?[faq]:[])]};
}

export function renderAziziFlorence({project,campaign,units=[],sources=[],seo={},origin=CANONICAL_ORIGIN,whatsappNumber=''}){
  const canonical=`${origin.replace(/\/$/,'')}${AZIZI_FLORENCE_PATH}`;
  const distribution=seo.distribution||{};
  const shareTitle=distribution.share_title||seo.title||'Azizi Florence | Finding Stories';
  const shareDescription=distribution.share_description||seo.description||'Verified project information and private advisor support for Azizi Florence.';
  const shareImage=distribution.share_image||seo.hero_image;
  ...
  const unitCards=verifiedUnits.map((x,index)=>`<article class="unit-card">${projectVisual(residenceAsset(x),'unit-visual',x.unit_type)}<div class="unit-body"><div class="unit-number">0${index+1}</div><p class="card-kicker">${esc(x.property_type||'Residence')}</p><h3>${esc(x.unit_type)}</h3><dl>${fact('Bedrooms',x.bedrooms)}${fact('Size',x.minimum_area!=null&&x.maximum_area!=null?`${area(x.minimum_area)} – ${area(x.maximum_area)}`:x.minimum_area!=null?`From ${area(x.minimum_area)}`:'')}${fact('Starting from',x.starting_price!=null?money(x.starting_price,x.price_currency||'AED'):'')}</dl><a href="#enquire" class="card-link" data-conversion="availability_request" data-unit="${esc(x.unit_type)}">Request current availability <span>↗</span></a></div></article>`).join('');
  const whatsAppMessage='Hi, I’m interested in Azizi Florence. Please share the latest availability, price and payment plan.';
  const whatsApp=whatsappNumber?`<a class="button button-outline" data-conversion="whatsapp" target="_blank" rel="noopener noreferrer" href="https://wa.me/${esc(whatsappNumber)}?text=${encodeURIComponent(whatsAppMessage)}"><span>Check on WhatsApp</span></a>`:'';
  const mobileWhatsApp=whatsappNumber?`<a data-conversion="whatsapp" target="_blank" rel="noopener noreferrer" href="https://wa.me/${esc(whatsappNumber)}?text=${encodeURIComponent(whatsAppMessage)}">WhatsApp</a>`:'';
  ...
  const schema=JSON.stringify(aziziStructuredData(project,verifiedUnits,origin,seo)).replaceAll('<','\\u003c');
  const faqSection=seo.faq?.length?`<section class="faq section-pad" id="faq"><div class="section-heading"><p class="eyebrow">BUYER QUESTIONS</p><h2>Azizi Florence<br><em>questions, answered.</em></h2><p>Clear guidance based on information currently verified by Finding Stories.</p></div><div class="faq-list">${seo.faq.map(item=>`<details><summary>${esc(item.question)}</summary><p>${esc(item.answer)}</p></details>`).join('')}</div><a class="button button-gold" href="#enquire" data-conversion="enquiry">Ask about Florence</a></section>`:'';
  ...
  // This slot is reserved for the audited map; never substitute decorative imagery.
  const locationImage=locationAsset;
  const locationViewer=locationImage?`<button class="location-visual" type="button" data-map-open aria-haspopup="dialog" aria-label="Open interactive Florence location map"><img src="${mediaUrl(locationImage)}" alt="Verified Florence location plan" loading="lazy" decoding="async"><span>Tap to explore map</span></button><div class="map-viewer" data-map-viewer role="dialog" aria-modal="true" aria-label="Interactive Florence location map" aria-hidden="true"><div class="map-toolbar" aria-label="Map controls"><button type="button" data-map-zoom-in aria-label="Zoom in">+</button><button type="button" data-map-zoom-out aria-label="Zoom out">−</button><button type="button" data-map-fit>Fit</button><button type="button" data-map-close>Close</button></div><div class="map-stage" data-map-stage><img src="${mediaUrl(locationImage)}" alt="Verified Florence location plan at full resolution" draggable="false"></div><p class="map-help">Scroll or pinch to zoom · Drag to pan · Double-click to zoom</p></div>`:'';
  const locationSection=(project.area||project.emirate||connections.length)?`<section class="location-section section-pad"><div class="location-panel"><p class="eyebrow">LOCATION</p><h2>Prime location in Sharjah</h2><p>Review the verified Florence location, direct road access and connectivity details available for this release.</p>${connections.length?`<ul>${connections.map(x=>`<li>${esc(x)}</li>`).join('')}</ul>`:''}<a class="inline-link" href="#enquire" data-engagement="location_explore">Explore location <span>↗</span></a></div>${locationViewer||`<aside class="location-editorial"><span>Florence</span><p>${esc([project.area,project.emirate].filter(Boolean).join(', '))}</p><small>Location details are verified privately with your advisor.</small></aside>`}</section>`:'';
  const paymentSection=project.payment_plan_summary?`<section class="payment section-pad"><div class="section-heading"><p class="eyebrow">PAYMENT PLAN</p><h2>Designed around<br><em>your journey.</em></h2><p>${esc(project.payment_plan_summary)}</p></div>${milestones.length?`<div class="timeline">${milestones.map((m,index)=>`<article><div class="timeline-value">${m.percentage}<sup>%</sup></div><div class="timeline-line"><span>${String(index+1).padStart(2,'0')}</span></div><h3>${esc(m.title||`Milestone ${index+1}`)}</h3></article>`).join('')}</div>`:`<div class="plan-statement"><strong>${esc(project.payment_plan_summary)}</strong><p>Ask an advisor for the milestone schedule.</p></div>`}<div class="payment-action"><p class="fine-print payment-note">Terms are subject to reconfirmation before commitment.</p><a class="button button-gold" href="#enquire" data-conversion="payment_plan_request">Get Payment Plan</a></div></section>`:'';
  const buyerGuide=`<section class="buyer-guide section-pad" id="buyer-guide"><div class="section-heading"><p class="eyebrow">BUYER INFORMATION</p><h2>Make a considered<br><em>Florence decision.</em></h2><p>Use the verified information on this page as a starting point. Finding Stories will reconfirm release-specific details before you make a commitment.</p></div><div class="buyer-guide-grid"><article id="floor-plans"><span>01</span><h3>Floor plans</h3><p>The verified collection comprises 3 and 4 bedroom townhouses plus 4, 5 and 6 bedroom villas. Approved layouts are shared for the residence you are considering; no unverified floor plan is displayed here.</p><a href="#enquire" title="Request a floor plan" data-conversion="brochure_request">Request Floor Plans →</a></article><article id="availability"><span>02</span><h3>Prices &amp; availability</h3><p>Starting prices and residence sizes shown above come from verified project records. They do not promise that a particular unit remains available; ask us to check the current release.</p><a href="#enquire" data-conversion="availability_request">Request current availability →</a></article><article id="eoi"><span>03</span><h3>Expression of interest</h3><p>Registering interest tells a Finding Stories advisor what you are considering so they can explain the current process and confirmed terms. It is not, by itself, an official EOI submitted to Azizi Developments.</p><a href="#enquire" data-conversion="enquiry">Register interest →</a></article><article id="investment"><span>04</span><h3>Investment considerations</h3><p>Consider the verified master-community scale, townhouse and villa mix, payment structure, green and open-space proposition, connectivity and fit with your own budget and time horizon. Returns, rental yield and appreciation are uncertain and must be assessed independently.</p><a href="#enquire" data-conversion="consultation">Speak with an advisor →</a></article>${handover?`<article id="handover"><span>05</span><h3>Completion</h3><p>The currently recorded expected handover is ${esc(handover)}. Ask an advisor to reconfirm the date and release terms before relying on them.</p><a href="#enquire" data-conversion="enquiry">Confirm completion details →</a></article>`:''}</div></section>`;
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(seo.title||'Azizi Florence | Finding Stories')}</title><meta name="description" content="${esc(seo.description||'Explore verified Azizi Florence project information and request current details from a Finding Stories advisor.')}"><link rel="canonical" href="${esc(canonical)}"><meta name="robots" content="index,follow,max-image-preview:large"><meta property="og:title" content="${esc(shareTitle)}"><meta property="og:description" content="${esc(shareDescription)}"><meta property="og:type" content="website"><meta property="og:site_name" content="Finding Stories"><meta property="og:url" content="${esc(canonical)}">${shareImage?`<meta property="og:image" content="${esc(origin)}${esc(shareImage)}"><meta property="og:image:width" content="1600"><meta property="og:image:height" content="900"><meta property="og:image:alt" content="${esc(seo.hero_alt||project.name)}">`:''}<meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="${esc(shareTitle)}"><meta name="twitter:description" content="${esc(shareDescription)}">${shareImage?`<meta name="twitter:image" content="${esc(origin)}${esc(shareImage)}">`:''}<script type="application/ld+json">${schema}</script><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@300;400;500;600&family=Italiana&display=swap" rel="stylesheet"><link rel="stylesheet" href="/azizi-florence.css"></head><body><header class="nav"><a href="/" class="brand" aria-label="Finding Stories home"><strong>FINDING STORIES</strong><small>REAL ESTATE WITH PERSPECTIVE</small></a><nav aria-label="Primary navigation"><a href="/#projects">Projects</a><a href="/#developers">Developers</a><a href="/#areas">Areas</a><a href="/#investment">Investment</a><a href="/#about">About Us</a><a href="/#insights">Insights</a><a href="/#contact">Contact</a><a href="#enquire" class="nav-cta" data-conversion="enquiry">Enquire Now <span>→</span></a></nav></header><main><section class="hero${heroImage?' has-hero-image':' no-hero-image'}">${projectVisual(heroImage,'hero-art','signature project view','eager')}<div class="hero-content"><p class="eyebrow">A NEW LANDMARK IN SHARJAH</p><h1>Azizi<br>Florence</h1><p class="hero-tagline">Elegant living. A brighter tomorrow.</p>${logo?`<img class="developer-logo" src="${mediaUrl(logo)}" alt="Azizi Developments">`:``}<p class="hero-lede">${esc(project.description||'A private introduction to Azizi Florence, guided by verified information and personal property advisory.')}</p><div class="actions"><a class="button button-gold" data-conversion="enquiry" href="#enquire">Request Details</a>${whatsApp}</div><div class="hero-trust"><span>✓ Verified project information</span><span>◇ Direct advisory support</span><span>⌂ UAE market expertise</span><span>↗ End-to-end guidance</span></div><p class="hero-note">Pricing, terms and availability reconfirmed on enquiry</p></div><a href="#overview" class="scroll-cue" aria-label="Explore Azizi Florence"><span>Scroll to explore</span><i>↓</i></a></section><section class="highlights" aria-label="Project highlights">${derivedUsps.map((value,index)=>factCard(['Master community','Residences','Open spaces','Connectivity','Lifestyle'][index],value,['◇','⌂','♧','↗','○'][index])).join('')||`${factCard('Residence mix',bedroomRange,'⌂')}${factCard('Starting from',lowestPrice!=null?money(lowestPrice):'','↗')}${factCard('Expected handover',handover,'◷')}`}</section><section class="overview section-pad" id="overview"><div class="overview-copy"><p class="eyebrow">PROJECT OVERVIEW</p><h2>A Master-Planned<br>Community for a Better You</h2><p>${esc(project.description||'Speak privately with our advisory team to review the verified Florence project information against your requirements.')}</p>${derivedUsps.length?`<ul class="overview-features">${derivedUsps.slice(0,4).map(x=>`<li>${esc(x)}</li>`).join('')}</ul>`:''}<a href="#residences" class="inline-link">Explore the residences <span>↓</span></a></div><div class="gallery${images.length?'':' gallery-empty'}">${projectVisual(assetMap.overview_main,'gallery-card gallery-1','The arrival')}${projectVisual(projectImages[1]||heroImage,'gallery-card gallery-2','The residences')}${projectVisual(projectImages[2]||projectImages[0]||heroImage,'gallery-card gallery-3','The details')}</div></section>${unitCards?`<section class="residences section-pad" id="residences"><div class="section-heading"><p class="eyebrow">THE COLLECTION</p><h2>Verified unit types<br>& starting prices.</h2><p>Explore the verified Florence collection. Request current release details and floor plans privately.</p></div><div class="unit-grid">${unitCards}</div></section>`:''}<div id="payment">${paymentSection}</div>${buyerGuide}${amenitySection}${locationSection}${faqSection}<section class="share-project section-pad" aria-labelledby="share-project-title"><div><p class="eyebrow">SHARE THE GUIDE</p><h2 id="share-project-title">Pass on verified<br><em>Florence context.</em></h2><p>Share the canonical buyer guide without unsupported claims or false urgency.</p></div><div class="share-actions"><a class="button button-outline" data-engagement="project_share_whatsapp" target="_blank" rel="noopener noreferrer" href="https://wa.me/?text=${encodeURIComponent(`${distribution.whatsapp_share_copy||shareDescription} ${canonical}?utm_source=whatsapp&utm_medium=whatsapp_share&utm_campaign=azizi-florence-project-launch&utm_content=onsite_share`)}">Share on WhatsApp</a><button class="button button-gold" type="button" data-copy-project-link data-engagement="project_link_copy">Copy project link</button><p data-copy-status role="status" aria-live="polite"></p></div></section><section class="enquiry section-pad" id="enquire"><div class="enquiry-intro"><p class="eyebrow">PRIVATE ADVISORY</p><h2>Register your<br><em>interest.</em></h2><p>Tell us what you are looking for. A Finding Stories advisor will contact you to confirm verified details, discuss suitability and explain possible next steps.</p><div class="advisor-promise"><span>01</span><p><strong>Personal guidance</strong>One advisor, focused on your brief.</p><span>02</span><p><strong>Verified information</strong>Facts reviewed before they reach you.</p><span>03</span><p><strong>No-pressure process</strong>Clarity before any commitment.</p></div><p class="eoi-note">This is an enquiry to Finding Stories, not an official developer EOI.</p><a class="inline-link light-link" href="#enquire" data-conversion="brochure_request">Request the brochure <span>↗</span></a></div><form id="azizi-lead-form"><input class="trap" name="website" tabindex="-1" autocomplete="off"><div class="form-title"><span>Private enquiry</span><small>Fields marked * are required</small></div><label>Full name *<input name="name" required minlength="2" autocomplete="name" placeholder="Your name"></label><label>Mobile number *<input name="phone" required minlength="7" inputmode="tel" autocomplete="tel" placeholder="+971 50 123 4567"></label><label>Email address *<input name="email" required type="email" autocomplete="email" placeholder="you@example.com"></label><label>Preferred residence *<select name="property_type" required><option value="">Please select</option>${verifiedUnits.map(x=>`<option>${esc(x.unit_type)}</option>`).join('')}<option value="Not sure">I would like guidance</option></select></label><label>Approximate budget *<input name="budget" required placeholder="AED"></label><label>Buying for *<select name="purpose" required><option value="">Please select</option><option value="Investment">Investment</option><option value="End use">End use</option></select></label><label>Purchase timeframe *<select name="purchase_timeline" required><option value="">Please select</option><option>Immediately</option><option>Within 3 months</option><option>3–6 months</option><option>6–12 months</option><option>Exploring</option></select></label><label>Preferred contact *<select name="preferred_contact_method" required><option value="">Please select</option><option>Phone</option><option>WhatsApp</option><option>Email</option></select></label><label class="wide">How can we help?<textarea name="additional_requirements" placeholder="Share any preferences or questions (optional)"></textarea></label><label class="consent wide"><input name="consent" type="checkbox" required><span>I agree to be contacted by Finding Stories about this enquiry. My details will be handled in line with the privacy policy.</span></label><input type="hidden" name="conversion_type" value="enquiry"><div class="form-actions wide"><button class="button button-gold" type="submit" data-conversion="enquiry">Register Interest</button><button class="text-button" type="button" data-form-submit data-conversion="consultation">Book Private Consultation</button><button class="text-button" type="button" data-form-submit data-conversion="site_visit">Request a site visit</button></div><p class="wide form-status" id="form-status" role="status" aria-live="polite"></p></form></section><section class="why section-pad"><div><p class="eyebrow">FINDING STORIES</p><h2>Why choose<br><em>Finding Stories?</em></h2></div><div class="why-grid"><article><span>01</span><h3>Verified information</h3><p>Clear project facts, carefully reviewed before they reach you.</p></article><article><span>02</span><h3>Personalised advisory</h3><p>Your goals, budget and timing shape every recommendation.</p></article><article><span>03</span><h3>UAE market expertise</h3><p>Local perspective to help you compare opportunities with confidence.</p></article><article><span>04</span><h3>End-to-end support</h3><p>Guidance from first enquiry through each appropriate next step.</p></article></div></section><section class="trust section-pad">${sourceTrust}</section></main><footer><div class="footer-brand"><span>FINDING</span><span>STORIES</span></div><p>UAE property advisory for decisions that deserve more care.</p><nav><a href="/">Home</a><a href="#enquire">Private enquiry</a><a href="/privacy">Privacy</a></nav><small>© ${new Date().getUTCFullYear()} Finding Stories. Project details are subject to confirmation.</small></footer><div class="mobile-cta"><a href="#enquire" data-conversion="enquiry">Enquire</a>${mobileWhatsApp}</div><script>window.__AZIZI_FUNNEL__=${JSON.stringify({page_url:AZIZI_FLORENCE_PATH,project_id:String(project.id),campaign_id:String(campaign.id),project:project.name,developer:project.developer,area:project.area||''}).replaceAll('<','\\u003c')}</script><script src="/azizi-florence.js" type="module"></script></body></html>`;
}
