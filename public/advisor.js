...
const advisorSubmissionId = crypto.randomUUID();
...
async function submitAdvisor(){ if(!document.querySelector('#ai-consent').checked){message('Please confirm consent before sending.');return;} const button=document.querySelector('#ai-send');if(button.disabled)return;button.disabled=true;button.textContent='Securely saving…'; try{const response=await fetch('/api/leads',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({...state,...tracking,source:'AI Property Advisor',consent:true,website:'',submission_id:advisorSubmissionId})});const result=await response.json();if(!response.ok)throw new Error(result.error);document.querySelector('.ai-form').innerHTML='';message(result.message || 'Thank you. We will be in touch.');sessionStorage.setItem('fs_lead_sent','1');}catch(error){button.disabled=false;button.textContent='Try again';message(error.message || 'Something went wrong. Please try again.');}}
