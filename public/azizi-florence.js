...
const conversionTypes=new Set(['enquiry','brochure_request','availability_request','consultation','site_visit','whatsapp']);
...
async function analytics(event_type,conversion_type){try{await fetch('/api/acquisition?route=events',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({event_key:crypto.randomUUID(),visitor_id:visitor,event_type,...(conversion_type?{conversion_type}:{}),page_url:context.page_url,page_type:'project',area:context.area,project:context.project,developer:context.developer,source:latest.source,referrer:latest.referrer,utm_source:latest.utm_source,utm_medium:latest.utm_medium,utm_campaign:latest.utm_campaign,utm_content:latest.utm_content,utm_term:latest.utm_term}),keepalive:true});}catch{/* Analytics failure must never block an enquiry. */}}
analytics('page_view');
document.addEventListener('click',event=>{const target=event.target.closest('[data-conversion],[data-engagement],[data-unit]');if(!target)return;const conversion=target.dataset.conversion;if(conversionTypes.has(conversion)){analytics('cta_click',conversion);form.elements.conversion_type.value=conversion;if(conversion==='whatsapp')analytics('whatsapp','whatsapp');}if(target.dataset.engagement)analytics(target.dataset.engagement);if(target.dataset.unit)form.elements.property_type.value=target.dataset.unit;});
let started=false;form.addEventListener('input',()=>{if(!started){started=true;analytics('enquiry_started');}},{once:true});
for(const button of form.querySelectorAll('[data-form-submit]'))button.addEventListener('click',()=>{form.elements.conversion_type.value=button.dataset.conversion;form.requestSubmit();});
form.addEventListener('submit',async event=>{event.preventDefault();const status=form.querySelector('#form-status');const values=Object.fromEntries(new FormData(form));const submissionKey=`fs_submission:${context.campaign_id}`;let submission_id=sessionStorage.getItem(submissionKey);if(!submission_id){submission_id=crypto.randomUUID();sessionStorage.setItem(submissionKey,submission_id);}const conversion=values.conversion_type||'enquiry';const payload=buildProjectLeadPayload({values,context,latest,first,submissionId:submission_id});status.textContent='Securely submitting your enquiry…';for(const control of form.elements)control.disabled=true;try{const response=await fetch('/api/leads',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});const data=await response.json();status.textContent=response.ok?data.message:data.error||'Please check your details and try again.';if(response.ok&&!data.duplicate){analytics(conversion,conversion);form.reset();sessionStorage.removeItem(submissionKey);}}catch{status.textContent='We could not submit your enquiry. Please try again.';}finally{for(const control of form.elements)control.disabled=false;}});
