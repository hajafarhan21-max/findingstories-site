...
function renderSearchConsole(data){
 const readiness=document.querySelector('#search-console-readiness');readiness.innerHTML=`<article class="ai-card"><span class="ai-badge">SEARCH CONSOLE: ${escapeHtml(data.connection.status)}</span><p>${escapeHtml(data.connection.message)}</p>${data.connection.missing.length?`<p><b>Required configuration:</b> ${escapeHtml(data.connection.missing.join(' · '))}</p>`:''}${data.data_notice?`<p class="escalations">${escapeHtml(data.data_notice)}</p>`:''}</article>`;
