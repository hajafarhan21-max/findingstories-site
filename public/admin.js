...
function render() {
  ...
  list.innerHTML = shown.map(lead => `<article class="lead ${isOverdue(lead.next_follow_up_at) ? 'overdue' : ''}" data-id="${lead.id}">
    ...
    <div class="detail hidden"><div class="detail-copy"><p><b>Conversion</b>${escapeHtml(statusLabel(lead.conversion_type || 'enquiry'))}</p><p><b>Requirement</b>${escapeHtml(lead.additional_requirements || lead.requirement_summary || 'Not specified')}</p><p><b>Qualification</b>${escapeHtml(lead.qualification_summary || 'Qualification in progress')}</p><p><b>Next action</b>${escapeHtml(lead.next_action || 'Not set')}</p><p><b>Residence</b>${escapeHtml([lead.property_type, lead.bedrooms, lead.preferred_areas].filter(Boolean).join(' · ') || 'Not specified')}</p><p><b>Buyer profile</b>${escapeHtml([lead.budget,lead.purpose,lead.purchase_timeline,lead.preferred_contact_method].filter(Boolean).join(' · ') || 'Not specified')}</p><p><b>Acquisition</b>${escapeHtml([lead.acquisition_project,lead.source,lead.medium,lead.utm_source,lead.utm_medium,lead.utm_campaign,lead.utm_content,lead.utm_term].filter(Boolean).join(' · ') || 'Not attributed')}</p><p><b>Last contacted</b>${escapeHtml(formatDubaiDateTime(lead.last_contacted_at))}</p><p><b>Meeting / site visit</b>${escapeHtml(formatDubaiDateTime(lead.meeting_at))} / ${escapeHtml(formatDubaiDateTime(lead.site_visit_at))}</p></div>${controls(lead)}</div>
...
let campaigns=[],campaignProjects=[],campaignUsers=[];
const pct=value=>`${Number(value||0).toFixed(1)}%`;
function renderCampaigns(){
 const target=document.querySelector('#campaign-list');target.innerHTML=campaigns.map(c=>`<article class="ai-card" data-campaign-id="${escapeHtml(c.id)}"><span class="ai-badge">${escapeHtml(c.status)}</span><span class="ai-priority">${escapeHtml(statusLabel(c.campaign_type))}</span><h3>${escapeHtml(c.name)}</h3><p><b>${escapeHtml(c.developer)}</b> · ${escapeHtml(c.project_name)} · ${escapeHtml(c.area||c.emirate||'Location not recorded')}</p><p>${c.metrics.actual.leads} / ${c.metrics.target.leads} leads · ${c.metrics.actual.eois} / ${c.metrics.target.eois} EOIs · AED ${Number(c.metrics.actual.revenue).toLocaleString()} / ${Number(c.metrics.target.revenue).toLocaleString()}</p><div class="row-actions"><button data-campaign-view>View</button>${currentRole==='SUPER_ADMIN'?'<button class="secondary" data-campaign-edit>Edit</button><button data-campaign-status="ACTIVE">Activate</button><button class="secondary" data-campaign-status="PAUSED">Pause</button><button class="danger" data-campaign-status="ARCHIVED">Archive</button>':''}</div></article>`).join('')||'<p class="empty">No production campaigns. No metrics have been fabricated.</p>';
}
async function loadCampaigns(){const response=await fetch('/api/admin/leads?crm=campaigns');if(!response.ok)return;const data=await response.json();campaigns=data.campaigns||[];campaignProjects=data.projects||[];campaignUsers=data.users||[];const form=document.querySelector('#campaign-form');form.project_id.innerHTML=campaignProjects.map(p=>`<option value="${escapeHtml(p.id)}">${escapeHtml(p.developer)} · ${escapeHtml(p.name)}</option>`).join('');form.owner_id.innerHTML='<option value="">Unassigned</option>'+campaignUsers.map(u=>`<option value="${escapeHtml(u.id)}">${escapeHtml(u.display_name)} · ${escapeHtml(u.role)}</option>`).join('');form.agent_ids.innerHTML=campaignUsers.map(u=>`<option value="${escapeHtml(u.id)}">${escapeHtml(u.display_name)}</option>`).join('');document.querySelector('#new-campaign').classList.toggle('hidden',currentRole!=='SUPER_ADMIN');renderCampaigns();}
function openCampaignForm(c){const form=document.querySelector('#campaign-form');form.reset();for(const [key,value] of Object.entries(c||{})){if(form.elements[key]&&key!=='agent_ids')form.elements[key].value=value??'';}[...form.agent_ids.options].forEach(option=>option.selected=(c?.agent_ids||[]).includes(option.value));form.classList.remove('hidden');form.name.focus();}
async function showCampaign(id){const response=await fetch(`/api/admin/leads?crm=campaigns&id=${encodeURIComponent(id)}`),data=await response.json();if(!response.ok)return;const c=data.campaign,m=c.metrics,a=m.actual,t=m.target,labels={leads:'Total Leads',qualified_leads:'Qualified',meetings:'Meetings',site_visits:'Site Visits',eois:'EOIs',bookings:'Bookings',revenue:'Revenue'};document.querySelector('#campaign-detail').innerHTML=`<hr class="workspace-divider"><h2>${escapeHtml(c.name)}</h2><p>${escapeHtml(c.developer)} · ${escapeHtml(c.project_name)} · ${escapeHtml(c.area||c.emirate||'Location not recorded')} · ${escapeHtml(c.status)} · ${escapeHtml(c.starts_on||'No start date')} — ${escapeHtml(c.ends_on||'No end date')} · Owner ${escapeHtml(c.owner_name||'Unassigned')}</p><div class="command-grid">${Object.entries(labels).map(([k,label])=>`<div class="command-card"><b>${k==='revenue'?'AED ':''}${Number(a[k]).toLocaleString()}</b><span>${label} · target ${Number(t[k]).toLocaleString()} · ${pct(m.conversion[k])}</span></div>`).join('')}</div><h3>50 EOI Command Center</h3><div class="command-grid">${Object.entries({target:m.eoi_command.target,completed:m.eoi_command.completed,pending:m.eoi_command.pending,conversion_percent:pct(m.eoi_command.conversion_percent),required_remaining:m.eoi_command.remaining}).map(([k,v])=>`<div class="command-card"><b>${escapeHtml(v)}</b><span>${escapeHtml(statusLabel(k))}</span></div>`).join('')}</div><h3>Recent leads</h3>${data.recent_leads.map(l=>`<p>${escapeHtml(l.name)} · ${escapeHtml(l.temperature)} · ${escapeHtml(l.status)}</p>`).join('')||'<p class="empty">No attributed production leads.</p>'}<h3>Upcoming follow-ups</h3>${data.upcoming_follow_ups.map(x=>`<p>${escapeHtml(x.title)} · ${escapeHtml(x.lead_name)} · ${escapeHtml(x.due_at||'Not scheduled')}</p>`).join('')||'<p class="empty">No upcoming follow-ups.</p>'}${metricsTable('Source performance',data.source_performance,[['name','Source'],['leads','Leads'],['qualified','Qualified'],['bookings','Bookings']])}${metricsTable('Agent performance',data.agent_performance,[['name','Agent'],['leads','Leads'],['qualified','Qualified'],['bookings','Bookings'],['revenue','Revenue']])}<h3>EOI pipeline</h3>${data.eoi_pipeline.map(e=>`<p>${escapeHtml(e.lead_name)} · ${escapeHtml(e.status)} · ${escapeHtml(e.completed_at||e.created_at)}</p>`).join('')||'<p class="empty">No persisted EOIs.</p>'}<p class="workspace-intro">Meetings, site visits and the conversion funnel above are calculated only from persisted attributed CRM records.</p>`;}
document.querySelector('#new-campaign').onclick=()=>openCampaignForm();document.querySelector('#cancel-campaign').onclick=()=>document.querySelector('#campaign-form').classList.add('hidden');
document.querySelector('#campaign-form').addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget,data=new FormData(form),id=data.get('id'),numeric=['target_leads','target_qualified_leads','target_meetings','target_site_visits','target_eois','target_bookings','target_revenue'];const body=Object.fromEntries(data);for(const key of numeric)body[key]=Number(body[key]||0);body.owner_id=body.owner_id||null;body.starts_on=body.starts_on||null;body.ends_on=body.ends_on||null;body.agent_ids=[...form.agent_ids.selectedOptions].map(x=>x.value);const response=await fetch('/api/admin/leads?crm=campaigns',{method:id?'PATCH':'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}),result=await response.json();document.querySelector('#campaign-message').textContent=response.ok?'Campaign saved.':result.error;if(response.ok){form.classList.add('hidden');await loadCampaigns();}});
document.querySelector('#campaign-list').addEventListener('click',async event=>{const card=event.target.closest('[data-campaign-id]');if(!card)return;const c=campaigns.find(x=>x.id===card.dataset.campaignId);if(event.target.closest('[data-campaign-view]'))return showCampaign(c.id);if(event.target.closest('[data-campaign-edit]'))return openCampaignForm(c);const status=event.target.closest('[data-campaign-status]')?.dataset.campaignStatus;if(status){await fetch('/api/admin/leads?crm=campaigns',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:c.id,status})});await loadCampaigns();}});

async function loadActionQueue() {
  const response = await fetch('/api/admin/leads?view=revenue'); if (!response.ok) return;
  const data = await response.json(); window.__recovery=data.recovery?.queue||[]; actionQueue = data.queue; propertyOpportunities=data.property_opportunities||[]; renderActionQueue(data.refreshing); renderPropertyOpportunities(data.inventory_count); renderCommandCenter(data); renderAcquisition(data.acquisition,data.acquisition_coverage); renderBinghattiAttribution(data.binghatti_attribution); renderPipeline(data.pipeline); renderRecovery(data.recovery); loadSearchConsole(); loadBinghattiActivation(); loadLaunchCommandCenter(); loadCampaigns();
}
