...
const files={
  'market-summary.json':snapshot,'top-area-movements.json':empty('CALCULATED_METRIC',{status:'unavailable'}),'notable-transactions.json':empty('FACT',{status:'unavailable'}),
  ...
  'news-candidates.json':empty('SOURCE_SUMMARY',{publicationState:'REVIEW_REQUIRED'}),'newsletter-draft.json':empty('EDITORIAL_INTERPRETATION',{status:'DRAFT_NOT_APPROVED'}),
  'homepage-intelligence.json':{generatedAt:new Date().toISOString(),status:snapshot.status,market:snapshot.records.length?snapshot:null}
};
for(const [name,value] of Object.entries(files))await writeFile(`${dir}/${name}`,`${JSON.stringify(value,null,2)}\n`);
await mkdir('generated/data-health',{recursive:true});await writeFile('generated/data-health/source-health.json',`${JSON.stringify({generatedAt:new Date().toISOString(),sources:[{id:'dld',status:snapshot.status,lastAttemptAt:snapshot.retrievedAt}]},null,2)}\n`);
...
await mkdir('generated/newsletter',{recursive:true});await writeFile('generated/newsletter/newsletter-draft.json',`${JSON.stringify(empty('EDITORIAL_INTERPRETATION',{status:'DRAFT_NOT_APPROVED'}),null,2)}\n`);
