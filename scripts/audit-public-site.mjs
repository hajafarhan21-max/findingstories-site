...
const counts={total_routes:records.length,populated_routes:records.filter(r=>r.status==='PASS').length,intentional_partial_routes:records.filter(r=>r.status==='INTENTIONALLY GATED').length,broken_routes:unique.length,manual_dld_datasets_discovered:0,datasets_successfully_mapped:0,project_guides_published:PUBLISHED_PROJECTS.length,areas_populated:records.filter(r=>r.route.startsWith('/areas/')&&r.status==='PASS').length,developers_populated:records.filter(r=>r.route.startsWith('/developers/')&&r.status==='PASS').length,property_type_pages_populated:records.filter(r=>r.route.startsWith('/property-types/')&&r.status==='PASS').length};try{const coverage=JSON.parse(await readFile('generated/mappings/dld-dataset-coverage.json','utf8'));counts.manual_dld_datasets_discovered=coverage.datasets.length;counts.datasets_successfully_mapped=coverage.datasets.filter(d=>d.validation?.status==='PASSED').length;counts.fields_mapped=coverage.datasets.reduce((n,d)=>n+Object.values(d.canonical_mappings??{}).filter(x=>x.length).length,0);counts.fields_unmapped=coverage.datasets.reduce((n,d)=>n+(d.unmapped_columns?.length??0),0)}catch{counts.fields_mapped=0;counts.fields_unmapped=0}await writeFile('generated/qa/site-audit.json',JSON.stringify({schema_version:2,...counts,links_checked:links.length,failures:unique,records},null,2)+'\n');
