...
let completed=false;
try{
  created=await submit();assert.equal(created.response.status,201,JSON.stringify(created.data));assert.equal(created.data.is_test,true);
  ...
  for(let attempt=0;attempt<12;attempt++){
    ...
    const candidate=inspection.data.rsvps.find(item=>item.id===created.data.id);
    if(candidate?.qualification_status==='completed'&&candidate.crm_lead_id)break;
    await new Promise(resolve=>setTimeout(resolve,2500));
  }
  const row=inspection.data.rsvps.find(item=>item.id===created.data.id);assert.equal(row?.is_test,true);assert.ok(row?.crm_lead_id);assert.equal(row?.qualification_status,'completed');
  ...
  if(created?.data?.id){
    const archived=await patch({action:'archive',rsvp_id:created.data.id});assert.equal(archived.response.ok,true,JSON.stringify(archived.data));assert.equal(archived.data.result.is_test,true);
    ...
    const row=inspection.data.rsvps.find(item=>item.id===created.data.id);assert.equal(row?.is_test,true);assert.ok(row?.archived_at,'synthetic TEST RSVP cleanup was not persisted');
  }
}
if(completed)console.log(JSON.stringify({ok:true,event_id:event.id,rsvp_id:created.data.id,checks:['qualification','crm_lead','meeting','site_visit','assignment','status','activity','report','export','archive']}));
