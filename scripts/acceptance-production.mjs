...
const event=available.data.event,slots=available.data.slots.filter(slot=>Number(slot.remaining)>=2);assert.ok(slots.length>=1,'a future TEST slot with capacity for two RSVPs is required');
...
try{
  ...
  for(const row of rows){assert.equal(row?.is_test,true);assert.ok(row?.crm_lead_id);assert.equal(row?.qualification_status,'completed');assert.ok(inspection.data.activities.some(activity=>activity.rsvp_id===row.id&&activity.activity_type==='rsvp_submitted'));}
  const row=rows[0];
  ...
  const report=await api(`/api/acceptance/events?event_id=${event.id}&action=report`);assert.equal(report.response.ok,true);assert.equal(report.data.is_test,true);assert.ok(Number(report.data.counts.total)>=2);
